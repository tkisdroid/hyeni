...
- [ ] **PAIR-01**: `pair_code`에 TTL(48시간 기본) + 부모 수동 회전 버튼이 존재한다. 만료된 코드로 join 시도 시 명확한 에러.
- [ ] **PAIR-02**: 한 가족에 `role='child'` 복수 row가 허용되더라도, 부모의 명시적 "아이 추가" 액션 없이 신규 익명 세션이 기존 `이름='아이'` slot을 덮어쓰지 않는다.
